...
bool point::approx_eq(const point& p) const 
{
  if((x-p.x)>0.005 || (x-p.x)<-0.0005)
    return false;
  if((y-p.y)>0.005 || (y-p.y)<-0.0005)
    return false;
...
/*float cross_prod(const point& p1, const point& p2, const point& p3)
{
...
{
  if(my_points.size()<=1) return true;
  //  if(my_points.size()==1) return my_points[0];
